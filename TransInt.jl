...
push!(LOAD_PATH, ".")
#push!(LOAD_PATH, "C:\\Users\\Maura\\Documents\\College\\CS 330\\Interpreter2")
...
function parse( expr::Array{Any} )
    ...
    elseif expr[1] == :+ && length(expr) > 3
        printlnD("New plus node")
        numExprList = OWL[]
        for i in 2:length(expr)
          push!(numExprList, parse(expr[i]))
        end
        return PlusNode(numExprList)
...
function analyze(ast::PlusNode)
  if length(ast.numExprList) == 2
    return Binop(
      opDict[:+],
      analyze(ast.numExprList[1]),
      analyze(ast.numExprList[2])
    )
  end
    ...
    opDict[:+],
    analyze(ast.numExprList[1]),
    analyze(PlusNode(ast.numExprList[2:length(ast.numExprList)]))
  )
end
...
function calc( ast::Binop, env::Environment )
    ...
    return NumVal(ast.op(lhsresult.n, rhsresult.n))
end
